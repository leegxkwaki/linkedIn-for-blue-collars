...
### 📊 주요 테이블 구조
* **`code_group` / `code_detail`**: 시스템 전체 유형 코드 관리.
* **`account_master`**: 이메일, 패스워드, 인증 토큰.
* **`user_info`**: 성함, 연락처, 사용자 역할 코드.
* **`worker_profiles`**: 숙련도, 주요 기술 코드, 경력 연수.
* **`post_master`**: 모든 게시물의 부모 (제목, 작성자, 타입).
* **`job_details`**: 구인공고 전용 정보 (급여, 위치 좌표, 마감일).
